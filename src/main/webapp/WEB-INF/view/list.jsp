...
$(document).ready(function(){
    ...
    if(checkUser == '' || checkUser==null){
    	...
    	$('#button').css('display','none');
    	$('#emptyUser').css('display','none');
    	$('#admin_header').css('display','inline-block');
...
<body>
    <header class="w3-container" id="admin_header" style="padding-top:22px;display: none;">
	...
	<td colspan="6" align='center'><p class="a">Man maketh manner</p></td>
		...
		<!-- 이전 출력 시작-->
		 <c:if test="${pv.startPage>1 }">
			<a href="qnaGo.do?currentPage=${pv.startPage-pv.blockPage}">◁</a>
   ...
   <!-- 페이지 출력 끝 -->
	<!-- 다음 출력 시작 -->
	 <c:if test="${pv.endPage<pv.totalPage}">
		<a href="qnaGo.do?currentPage=${pv.startPage+pv.blockPage}&searchKey=${pv.searchKey}&searchWord=${pv.searchWord}">▶</a>	
	</c:if> 
	<!-- 다음 출력 끝 -->
	 </div> 	
